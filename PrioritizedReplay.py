...
class PrioritizedReplay():
    def __init__(self, capacity, batch_size, n_features, buffer_type='priority'):
    ...
    def add(self,  state, action, reward, state2):
        ...
        transition = np.hstack((state, [action, reward], state2))
    ...
    def sample(self):
        ...
        if self.type == 'priority':
            ...
            sample_index = np.random.choice(self.memory_size, size=self.batch_size)     # 考虑buffer已先填满
