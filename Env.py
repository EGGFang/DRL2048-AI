...
class Game2048Env(gym.Env):   # directions 0, 1, 2, 3 are up, right, down, left
    ...
    def draw_board_set(self):
        self.WINDOW_SIZE = (370, 370)
    ...
    def draw_board(self, board):
        ...
        for row in range(4):
            for col in range(4):
                ...
                tile_color = self.TILE_COLORS.get(tile_value, self.TILE_COLORS[0])
                tile_rect = pygame.Rect(
                    col * (self.TILE_SIZE + self.TILE_MARGIN),
                    row * (self.TILE_SIZE + self.TILE_MARGIN),
                    self.TILE_SIZE,
                    self.TILE_SIZE
                )
